import type { CurrentUserData } from '../../../auth/utils/current-user-decorator';
import { ListPullReviewCommentsUseCase } from './list-pull-review-comments.use-case';

const currentUser: CurrentUserData = {
  id: 'user-1',
  username: 'octocat',
  email: 'octocat@example.com',
};

function fakeGithubSession(octokit: any, owner = 'octocat') {
  return {
    getSession: jest.fn().mockResolvedValue({ octokit, owner }),
    resolveOwner: jest.fn(
      (session, ownerOverride) => ownerOverride?.trim() || session.owner,
    ),
    handleGithubError: jest.fn((err) => {
      throw err;
    }),
  } as any;
}

describe('ListPullReviewCommentsUseCase', () => {
  it('maps paginated review comments to id/body/user', async () => {
    const octokit = {
      pulls: { listReviewComments: jest.fn() },
      paginate: jest
        .fn()
        .mockResolvedValue([
          { id: 1, body: 'nit', user: { login: 'octocat' } },
        ]),
    };
    const useCase = new ListPullReviewCommentsUseCase(
      fakeGithubSession(octokit),
    );

    const result = await useCase.execute({
      repo: 'hello-world',
      pullNumber: 7,
      currentUser,
    });

    expect(octokit.paginate).toHaveBeenCalledWith(
      octokit.pulls.listReviewComments,
      { owner: 'octocat', repo: 'hello-world', pull_number: 7, per_page: 100 },
    );
    expect(result).toEqual([{ id: 1, body: 'nit', user: 'octocat' }]);
  });

  it('defaults user to null when the comment has no author', async () => {
    const octokit = {
      pulls: { listReviewComments: jest.fn() },
      paginate: jest
        .fn()
        .mockResolvedValue([{ id: 1, body: 'nit', user: null }]),
    };
    const useCase = new ListPullReviewCommentsUseCase(
      fakeGithubSession(octokit),
    );

    const result = await useCase.execute({
      repo: 'hello-world',
      pullNumber: 7,
      currentUser,
    });

    expect(result).toEqual([{ id: 1, body: 'nit', user: null }]);
  });

  it('delegates github errors to the session handler', async () => {
    const err = { status: 500 };
    const octokit = {
      pulls: { listReviewComments: jest.fn() },
      paginate: jest.fn().mockRejectedValue(err),
    };
    const githubSession = fakeGithubSession(octokit);
    const useCase = new ListPullReviewCommentsUseCase(githubSession);

    await expect(
      useCase.execute({ repo: 'hello-world', pullNumber: 7, currentUser }),
    ).rejects.toEqual(err);
    expect(githubSession.handleGithubError).toHaveBeenCalledWith(err);
  });
});
