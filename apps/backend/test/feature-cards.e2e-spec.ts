...
describe('Feature Cards HTTP and Postgres', () => {
  ...
  beforeAll(async () => {
    ...
    await db.query(
      `INSERT INTO users (id, name, email, password) VALUES ($1, 'Cards test', $2, 'test')`,
      [ownerId, `${ownerId}@test.invalid`],
    );
    await db.query(
      `INSERT INTO projects (id, owner_id, name) VALUES ($1, $2, 'Cards test')`,
      [projectId, ownerId],
    );
    await db.query(
      `INSERT INTO chat_threads (id, user_id, project_id, scope_type, title, scope) VALUES ($1, $2, $3, 'project', 'Feature', $4)`,
      [
        threadId,
        ownerId,
        projectId,
        JSON.stringify({ mode: 'project', projectId, repositories: [] }),
      ],
    );
    await db.query(
      `INSERT INTO chat_messages (id, thread_id, role, content, proposal) VALUES ($1, $2, 'assistant', 'Proposta', $3)`,
      [messageId, threadId, JSON.stringify(proposalFixture())],
    );
    const module = await Test.createTestingModule({
      ...
      providers: [
        ...
        { provide: AppLogger, useValue: { error: jest.fn() } },
        {
          ...
          useValue: {
            getById: async (id: string, user: { id: string }) => {
              if (id !== projectId || user.id !== ownerId)
    ...
    app.use((req, _res, next) => {
      req.user = {
        id: req.headers['x-test-user'] ?? ownerId,
  ...
  it('saves concurrently without duplicates and persists dependency IDs', async () => {
    ...
    const responses = await Promise.all([
      request(app.getHttpServer()).post(url).send({ messageId }),
      request(app.getHttpServer()).post(url).send({ messageId }),
    ]);
    ...
    expect(cards.find((c) => c.taskKey === 'ui')!.dependsOn).toEqual([
      cards.find((c) => c.taskKey === 'api')!.id,
    ]);
  });
  ...
  it('validates DTOs and rejects access outside the project', async () => {
    ...
    await request(app.getHttpServer())
      ...
      .expect(404);
    await request(app.getHttpServer())
      .post(`${root}/from-message`)
      .send({ messageId: 'invalid' })
      .expect(400);
    ...
    await request(app.getHttpServer())
      .patch(`${root}/${card.id}`)
      .set('x-test-user', randomUUID())
      .send({ version: 1, title: 'Invadido' })
      .expect(404);
    await request(app.getHttpServer())
      .patch(`${root}/${card.id}`)
      .send({ version: 1, status: 'unknown' })
      .expect(400);
  });
  ...
  it('paginates without duplicating or omitting cards', async () => {
    ...
    const second = await request(app.getHttpServer())
      .get(`${url}&after=${first.body.nextCursor}`)
      .expect(200);
    expect(first.body.items).toHaveLength(2);
    expect(second.body.items).toHaveLength(1);
    expect(second.body.nextCursor).toBeNull();
    expect(
      new Set([...first.body.items, ...second.body.items].map((c) => c.id))
        .size,
    ).toBe(3);
  });
  ...
  it('serializes edits, rejects stale versions, enforces dependencies and returns history', async () => {
    ...
    await request(app.getHttpServer())
      .patch(`${root}/${api.id}`)
      .send({ version: 3, status: 'ready' })
      .expect(400);
    const history = await request(app.getHttpServer())
      .get(`${root}/${api.id}/history`)
      .expect(200);
  ...
  it('archives a family atomically and does not resurrect it on repeated save', async () => {
    const parent = await db
      ...
      .findOneByOrFail({ taskKey: 'feature' });
    const root = `/projects/${projectId}/cards`;
    await request(app.getHttpServer())
      .post(`${root}/${parent.id}/archive`)
      .send({ version: parent.version })
      .expect(204);
