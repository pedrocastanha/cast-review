  ...
  return (
    <aside className="border-b border-border bg-surface/90 backdrop-blur-md lg:flex lg:min-h-screen lg:flex-col lg:border-r lg:border-b-0">
      <div className="flex items-center justify-between gap-4 px-5 py-4 lg:block lg:px-6 lg:py-7">
        <Link to="/projects" className="inline-flex min-h-11 items-center font-display text-base font-semibold tracking-tight text-ink">
          CAST<span className="text-accent">·</span>REVIEW
        </Link>
        <button type="button" onClick={logout} className="inline-flex min-h-11 min-w-11 items-center justify-center text-sm text-ink-dim transition-colors hover:text-ink lg:hidden">
          Sair
        </button>
      </div>
      ...
      <nav aria-label="Navegação principal" className="flex gap-1 overflow-x-auto border-t border-border px-3 py-2 lg:flex-col lg:border-0 lg:px-3 lg:py-0">
        ...
        >
          <span aria-hidden="true" className="grid size-5 place-items-center rounded-sm border border-current font-mono text-[10px]">04</span>
          Configurações
        </NavLink>
      </nav>
      ...
      <div className="mt-auto hidden border-t border-border p-5 lg:block">
        ...
        <button type="button" onClick={logout} className="inline-flex min-h-11 min-w-11 items-center justify-center text-sm text-ink-dim transition-colors hover:text-ink">Sair</button>
      </div>
    </aside>
  );
}
